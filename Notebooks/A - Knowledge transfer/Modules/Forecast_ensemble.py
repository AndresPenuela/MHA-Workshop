...
#    inflow_forecast.on_hover(on_hover_1a)
    ...
    demand_forecast = plt.plot(np.arange(1,N+1),d_for,colors=['gray'],stroke_width = 4,opacities = [0.4]*members_num,
...
#    sel_inflow_3 = plt.plot(np.arange(1,N+1),I_sel,scales={'x': x_sc_1, 'y': y_sc_1}, colors=['darkblue'],stroke_width = 4,line_style = 'solid',marker = None,marker_size = 20,labels = ['forecast'], display_legend = True)
    ...
    fig_3a       = plt.Figure(marks = [inflow_forecast_3,act_inflow_3], title = 'Inflows in the last 8 weeks', 
                        axes=[x_ax_1, y_ax_1],layout={'min_width': '900px', 'max_height': '300px'},
                        scales={'x': x_sc_1, 'y': y_sc_1},legend_location = 'bottom-left')
    
    demand_forecast_3 = plt.plot(x=np.arange(1,N+1),y=d_for,colors=['gray'],stroke_width = 4,opacities = [0.4]*members_num,
                               display_legend=False,scales={'x': x_sc_1, 'y': y_sc_1})
#    sel_demand_3 = plt.plot(np.arange(1,N+1),d_sel,scales={'x': x_sc_1, 'y': y_sc_1},colors=['darkolivegreen'],stroke_width = 4,line_style = 'solid',label = 'selected',marker = None,marker_size = 20,labels = ['forecast'], display_legend = True)
    ...
    fig_3b       = plt.Figure(marks = [demand_forecast_3,act_demand_3], title = 'Demand in the last 8 weeks', axes=[x_ax_1, y_ax_1],
                        layout={'min_width': '900px', 'max_height': '300px'},
                        scales={'x': x_sc_1, 'y': y_sc_1},legend_location = 'bottom-left')
...
def Forecast_ensemble(N,members_num,I_for,d_for):
    ...
    inflow_forecast_9 = plt.plot(x=np.arange(1,N+1),y=I_for,colors=['deepskyblue'],stroke_width = 4,opacities = [0.4]*members_num,
                               display_legend=False,scales={'x': x_sc_1, 'y': y_sc_1})
    fig_9a       = plt.Figure(marks = [inflow_forecast_9], title = 'Inflow forecast for the next 8 weeks', 
                        axes=[x_ax_1, y_ax_1],layout={'min_width': '900px', 'max_height': '300px'},
                        scales={'x': x_sc_1, 'y': y_sc_1},legend_location = 'bottom-left')
    
    demand_forecast_9 = plt.plot(x=np.arange(1,N+1),y=d_for,colors=['gray'],stroke_width = 4,opacities = [0.4]*members_num,
                               display_legend=False,scales={'x': x_sc_1, 'y': y_sc_1})
